        ...
        <v-card class="stat-card">
          <v-card-text>
            <div class="stat-icon-wrapper blue">
              <v-icon icon="mdi-certificate" size="32" />
            </div>
            <div class="stat-content">
              <div class="stat-value">{{ certifications.length }}</div>
              <div class="stat-label">Total de Certificações</div>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
...
const saveItem = async () => {
  saving.value = true
  try {
    const { _id, createdAt, updatedAt, __v, month, year, ...certificationData } = editedItem.value as any
