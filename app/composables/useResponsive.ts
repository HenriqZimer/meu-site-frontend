...
export const useResponsive = () => {
  ...
  const isSm = computed(
    () => screenSize.value.width >= BREAKPOINTS.sm && screenSize.value.width < BREAKPOINTS.md
  )
  const isMd = computed(
    () => screenSize.value.width >= BREAKPOINTS.md && screenSize.value.width < BREAKPOINTS.lg
  )
  const isLg = computed(
    () => screenSize.value.width >= BREAKPOINTS.lg && screenSize.value.width < BREAKPOINTS.xl
  )
  ...
  const mediaQuery = (breakpoint: keyof Breakpoints, direction: 'up' | 'down' = 'up') => {
    ...
    return direction === 'up' ? `(min-width: ${value}px)` : `(max-width: ${value - 1}px)`
  }
